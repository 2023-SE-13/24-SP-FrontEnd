    ...
    <div class="post-list">
        ...
        <div class="right-box scrollable">
            ...
            <template v-if="applyList.length > 0">
                <ApplyUnit @showInfo="handleShowInfo" v-for="(apply, index) in applyList" :key="index"
                    :apply-data="apply"></ApplyUnit>
            </template>
            <template v-else>
                <img src="@/assets/empty.png" alt="No posts" class="empty-image" />
            </template>
...
import { getPositionList, createPost, deletePost, getPostApply, editPost } from '@/api/api'
export default {
    data() {
        return {
            postList: [],
